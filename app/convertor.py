...
def currency_convertor(currency_from,currency_to,amount):
    ...
    if currency_from  in currency_list and currency_to in currency_list:
        ...
        print("Currency choice is invalid")

def exchange_rates(currency_from,currency_to):
    currency_list = currency_options()
    if currency_from  in currency_list and currency_to in currency_list:
        exchange_rate=float(rates[currency_to])/float(rates[currency_from])
        return exchange_rate
    else:
        print("Sorry, not a valid currency, try again")

def currency_options():
    currency_list = rates.keys()
    return currency_list


if __name__ == "__main__":
    base_currency = input("Please enter a base currency;")
    to_currency = input("Please enter a currency to conver to")
    amount = input("Please specify an amount for unit currency")
    currency_list = rates.keys()

    if base_currency not in currency_list:
        print("Sorry, the base currency is invalid")
        exit()

    if to_currency not in currency_list:
        print("Sorry, the to currency is invalid")
...
#adding note for travis testing